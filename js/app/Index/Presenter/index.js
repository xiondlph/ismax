App.Index.View.Index = {
  ...
  Signup: Backbone.View.extend({
    ...
    submit: function(e){
      var me = this;
      var bValid = true;
      me.updateTips(null, 'Регстрация...');
      bValid = bValid && me.checkLength(me.$el.find('input[name="email"]'), 1, 20, 'Укажите Email');
      bValid = bValid && me.checkRegexp(me.$el.find('input[name="email"]'), /^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?$/i, "Неверный формат Email" );
      if(bValid){
        me.$el.find('.loader').css({visibility: 'visible'});
        me.$el.find('input').attr('disabled', 'disabled');
    ...
    updateTips: function(input, invalid){
      this.$el.find('.invalid').each(function(index, el) {
        $(this).removeClass('invalid');
      });
      this.$el.find('.info').addClass('highlight');
      this.$el.find('.info').text(invalid);
      this.$el.find('.info').removeClass('highlight', 1500);
      if(input){
        input.addClass('invalid');
      }
    },

    checkLength: function(input, min, max, invalid){
      if (input.val().length > max || input.val().length < min){
        this.updateTips(input, invalid);
        return false;
      } else {
        return true;
      }
    ...
    checkRegexp: function(input, regexp, invalid){
